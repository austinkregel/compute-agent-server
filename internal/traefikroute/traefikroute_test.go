...
func TestWriteRouterConfig_Shape(t *testing.T) {
	...
	r, ok := got.HTTP.Routers["compute-agent-server"]
	if !ok {
		t.Fatal("router \"compute-agent-server\" not present")
	}
	wantRule := "Host(`monitor.kratos.kregel.dev`)"
	if r.Rule != wantRule {
		t.Errorf("rule = %q, want %q", r.Rule, wantRule)
	}
	if r.Service != "compute-agent-server" {
		t.Errorf("service = %q, want %q", r.Service, "compute-agent-server")
	}
	if r.TLS == nil || r.TLS.CertResolver != certResolverName {
		t.Errorf("tls.certResolver = %+v, want %q", r.TLS, certResolverName)
	}

	svc, ok := got.HTTP.Services["compute-agent-server"]
	if !ok {
		t.Fatal("service \"compute-agent-server\" not present")
	}
...
func TestWriteRouterConfig_Overwrites(t *testing.T) {
	...
	wantRule := "Host(`monitor.kratos.kregel.dev`)"
	if got.HTTP.Routers["compute-agent-server"].Rule != wantRule {
		t.Errorf("rule after overwrite = %q, want %q", got.HTTP.Routers["compute-agent-server"].Rule, wantRule)
	}
}
